...
<title>${siteName!""}|角色管理-添加角色</title>
                  ...
                  <table class="table table-striped">
                    ...
                    <tbody>
                      ...
                      <tr>
                        <td>
                          <label class="lyear-checkbox checkbox-primary">
                            <input name="menuId" type="checkbox" class="checkbox-parent" dataid="id-${topMenu.id}" value="${topMenu.id}">
                      ...
                      <#if secondMenu.parent == topMenu.id>
                      <tr>
                        <td class="p-l-20">
                          <label class="lyear-checkbox checkbox-primary">
                            <input name="menuId" type="checkbox" class="checkbox-parent checkbox-child" dataid="id-${topMenu.id}-${secondMenu.id}" value="${secondMenu.id}">
                      ...
                      <tr>
                        <td class="p-l-40">
                           <#list thirdMenus as thirdMenu>
                      	   <#if thirdMenu.parent == secondMenu.id>
                          <label class="lyear-checkbox checkbox-primary checkbox-inline">
                            <input name="menuId" type="checkbox" class="checkbox-child" dataid="id-${topMenu.id}-${secondMenu.id}-${thirdMenu.id}" value="${thirdMenu.id}">
...
$(document).ready(function(){
	...
	$("#add-form-submit-btn").click(function(){
        ...
        if (name.length==0){
            showErrorMsg('角色名不能为空!');
            return;
        }
		if (name.length<2){
            showErrorMsg('角色名称长度不能少于2!');
            return;
        }
		...
		$.ajax({
			...
			success:function(data){
				if(data.code == 0){
					showSuccessMsg('角色添加成功!',function(){
